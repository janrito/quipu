...
let tagsToDraw = $derived(
  tags
    ...
    .sort((a, b) =>
      a.isParent === b.isParent ? a.name.localeCompare(b.name) : a.isParent ? 1 : -1
    )
);
</script>
...
<div
  ...
  onkeydown={e => e.key === "Enter" && openBookmark()}
  onclick={preventDefault(openBookmark)}>
  {#if closeEnabled}
    <div class="z-10 -mb-5 ml-5 hidden h-5 flex-row justify-end pl-1 group-hover/bookmark:flex">
      <div
        role="button"
        tabindex="0"
        onkeydown={e => e.key === "Enter" && closeBookmark()}
        onclick={stopPropagation(preventDefault(closeBookmark))}
      ...
      {:else}
        <div class="-mt-1 ml-1 h-4 w-4">
          <span
            role="button"
            tabindex="0"
            onkeydown={e => e.key === "Enter" && highlightBookmark()}
            onclick={stopPropagation(preventDefault(highlightBookmark))}
